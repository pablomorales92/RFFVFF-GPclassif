function gp_model = GPC_RFF_train(X,y,options)
...
%    - thr: Threshold for convergence. Default = 0,005.
%    - D (or W, see lines 20-26 of the code): Number of Fourier Features (or explicit (Dxd) matrix of Fourier frequencies). 
...
%% Initializing some frequent variables, theta, W and
[n,d] = size(X);
...
for iter = 1:options.maxiter
    ...
    thetaLog = minimize(log(theta),@minus_log_prob,-options.maxiter_par,XW,v,g_xi);
    theta = exp(thetaLog);
    %% Check convergence
    theta_c = norm(theta-theta_old)/norm(theta_old);
    fprintf('%i \t %.7f  ||| %.7f \t %.7f \n',[iter,theta_c,theta(1),theta(2)]);
    if (theta_c< options.thr)
        if(~stuck)
...
%% Outputs
gp_model.W = W;
gp_model.theta = theta;
gp_model.xi = xi;
gp_model.L = L;
gp_model.mu_beta = mu_beta;
%% Only for checking approximations in the experiment
gp_model.Z = Z;
end
...
function [f,df] = minus_log_prob(thetaLog,XW,v,g_xi)
[n D] = size(XW);
theta = exp(thetaLog(1:2));
Z = zeros(n,2*D);
Z(:,1:2:(2*D)) = (theta(1)/realsqrt(D))*cos(XW./theta(2));
Z(:,2:2:(2*D)) = (theta(1)/realsqrt(D))*sin(XW./theta(2));
...
df = zeros(size(thetaLog));
dZd2 = zeros(size(Z));
dZd2(:,1:2:(2*D)) = Z(:,2:2:(2*D)).*XW./theta(2);
dZd2(:,2:2:(2*D)) = -Z(:,1:2:(2*D)).*XW./theta(2);
df(1) = 2*(aux1'*aux1)-2*sum(aux4.*aux4./(2*g_xi))-2*(aux3(:)'*Z(:));
df(2) = 2*((v'*dZd2)*aux2)-2*((aux4'*dZd2)*aux2)-2*(aux3(:)'*dZd2(:));
df = -df;
end
